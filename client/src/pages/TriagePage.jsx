...
const TriagePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (cached) {
      setResult({ ...cached, fromCache: true, local: true });
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!data || !data.triageResult || !data.triageResult.condition || !data.triageResult.urgency || !data.triageResult.action) {
        throw new Error('Server returned incomplete data');
      }
      setResult(data);
      setLocalCache(cacheKey, data, 3600); // 1 hour TTL
    } catch (err) {
      // 3. If offline, fallback to local cache
      if (!navigator.onLine) {
        const offlineCached = getLocalCache(cacheKey);
        if (offlineCached) {
          setResult({ ...offlineCached, fromCache: true, local: true, warning: 'Offline mode: showing cached result.' });
          setLoading(false);
          return;
        }
      }
